...
impl Provider<Repository<UserEntity>> for UserRepositoryProvider {
    ...
    ) -> injectable_rs_runtime::InjectableResult<Repository<UserEntity>> {
...
impl injectable_rs_runtime::Injectable for Repository<UserEntity> {
...
impl Provider<Repository<ProductEntity>> for ProductRepositoryProvider {
    async fn provide(
        ctx: &injectable_rs_runtime::ResolveContext,
    ) -> injectable_rs_runtime::InjectableResult<Repository<ProductEntity>> {
        let db: Arc<Db> = ctx.extract().await?;
        Ok(Repository {
            db,
            _phantom: PhantomData,
...
impl injectable_rs_runtime::Injectable for Repository<ProductEntity> {
