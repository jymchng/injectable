...
fn test_graph_circular_dependency_detection() {
    ...
    let has_cycle = errors.iter().any(|e| {
        matches!(
            e,
            injectable_rs_graph::ValidationError::CircularDependency { .. }
        )
    });
...
fn test_graph_missing_dependency() {
    ...
    let has_missing = errors.iter().any(|e| {
        matches!(
            e,
            injectable_rs_graph::ValidationError::MissingDependency { .. }
        )
    });
...
fn test_validation_error_display() {
    let err = injectable_rs_graph::ValidationError::CircularDependency {
...
fn test_validation_error_missing() {
    let err = injectable_rs_graph::ValidationError::MissingDependency {
        source: "Service".into(),
...
fn test_scope_singleton_depends_on_transient_is_error() {
    ...
    let has_scope_mismatch = errors
        .iter()
        .any(|e| matches!(e, injectable_rs_graph::ValidationError::ScopeMismatch { .. }));
    assert!(
        has_scope_mismatch,
...
fn test_scope_mismatch_identifies_both_scopes() {
    ...
    let scope_mismatch = errors.iter().find_map(|e| match e {
        injectable_rs_graph::ValidationError::ScopeMismatch {
            source,
            source_scope,
            dependency,
            dependency_scope,
...
fn test_scope_mismatch_display_message() {
    let err = injectable_rs_graph::ValidationError::ScopeMismatch {
...
fn test_scope_multiple_mismatches_detected() {
    ...
    let mismatch_count = errors
        .iter()
        .filter(|e| matches!(e, injectable_rs_graph::ValidationError::ScopeMismatch { .. }))
        .count();
    assert_eq!(
        mismatch_count, 2,
...
//
// Arc<Svc> field:
//   • Routes through `impl<T: Injectable> Extract for Arc<T>` in injectable_rs_runtime
