...
/// Returns `true` if `path` is one of the known path shapes for `injectable_rs_runtime::Inject`:
///
/// | Written as | segments |
/// |---|---|
/// | `Inject<T>` | `["Inject"]` |
...
fn is_known_inject_path(path: &syn::Path) -> bool {
    ...
    match s.as_slice() {
        [a] => a == "Inject",
        [a, b] => (a == "injectable" || a == "injectable_rs_runtime") && b == "Inject",
        _ => false,
    }
}
...
/// Check if a type is `injectable_rs_runtime::Inject<T>` (any recognised path form) and return T.
///
