...
impl InjectableArcFactory {
    ...
    ///
    /// Both arguments are plain `fn` pointers, making this a `const fn` so
    /// that `inventory::submit!` can place the result in a static initializer.
...
///
/// Submitted by:
/// - `#[injectable_impl]` (no `#[constructor]`) — direct method call wrappers
